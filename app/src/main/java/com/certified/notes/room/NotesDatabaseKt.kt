    ...
    companion object {
        ...
        private val roomCallback: Callback = object : Callback() {
            override fun onCreate(db: SupportSQLiteDatabase) {
                ...
                val note1 = Note("MTS 315", "Dynamic intent resolution", "Wow, intents allow components to be resolved at runtime")
                val note2 = Note("CPE 301", "Parameters", "Leverage variable-length parameter lists?")
                val note3 = Note("EEE 301", "Delegating intents", "PendingIntents are powerful; they delegate much more than just a component invocation")
                val note4 = Note("AGE 301", "Compiler options", "The -jar option isn't compatible with with the -cp option")
                val note5 = Note("EEE 305", "Serialization", "Remember to include SerialVersionUID to assure version compatibility")
                val note6 = Note("PMT 301", "Service default threads", "Did you know that by default an Android Service will tie up the UI thread?")
                val note7 = Note("EEE 311", "Anonymous classes", "Anonymous classes simplify implementing one-use types")
                val note8 = Note("EEE 307", "Long running operations", "Foreground Services can be tied to a notification icon")
                ...
                val todo2 = Todo("Learn LiveData, Room, ViewModel, WorkManager, Notification, Paging library", false)
                val todo3 = Todo("The above should be learnt completely in order to pass the AAD exam", false)
                val todo4 = Todo("Stop all projects that aren't related to the AAD exam", false)
                val todo5 = Todo("Visit the code labs and study guide for the AAD exam frequently", false)
                val todo6 = Todo("Practice, Practice and keep practicing. Obtaining the AAD certificate should be the only priority for now", false)
...
//                    notesDao.deleteAllTodos()
        ...
        fun getInstance(context: Context): NotesDatabaseKt? {
            if (instance == null) {
                instance = Room.databaseBuilder(context.applicationContext, NotesDatabaseKt::class.java, "notes_database")
