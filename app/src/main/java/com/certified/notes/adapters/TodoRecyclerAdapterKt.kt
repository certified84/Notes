...
class TodoRecyclerAdapterKt(val context: Context, val viewModel: NotesViewModel) :
    ListAdapter<Todo, TodoRecyclerAdapterKt.ViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): TodoRecyclerAdapterKt.ViewHolder {
        val itemView = LayoutInflater.from(context).inflate(R.layout.list_item_todos, parent, false)
        return ViewHolder(itemView)
    }

    override fun onBindViewHolder(holder: TodoRecyclerAdapterKt.ViewHolder, position: Int) {
